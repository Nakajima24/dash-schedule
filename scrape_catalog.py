...
def main():
    ...
    try:
        ...
        index_page, depts = "", []
        print(f"Catalog page fetch failed: {exc}", file=sys.stderr)
    if not depts:
        # Cloudflare routinely blocks the fetch from GitHub's datacenter IPs
        # (curl_cffi is 403'd and the browser fallback can't clear it there).
        # If a good catalog.json is already committed, that's a no-op, not a
        # failure — exit clean so the monthly run doesn't show red. Only fail
        # when there's nothing to fall back on (the very first build, which
        # should be run from a residential IP and committed).
        keep = os.path.exists(out_path)
        print("Could not read departments from the catalog page "
              f"(blocked from this network?). {'Keeping' if keep else 'No'} "
    ...
    # Detail pages are fetched a few at a time — the run is otherwise bound by
    # sequential network round-trips (~1000+ pages). Kept modest so we stay
    # polite and don't trip Cloudflare's rate limiting. The browser fallback
    # in fetch() isn't thread-safe, but it only engages when curl_cffi is
    # blocked (GitHub runners), and there the run has already bailed above.
    workers = int(os.environ.get("CATALOG_WORKERS", "8"))

    def fetch_detail(row):
        """(description, prerequisites, advisories) for one course, or Nones.

        Detail pages go straight through curl_cffi rather than the shared
        fetch(): with thousands of pages a transient Cloudflare 403 (rate
        limiting) must just skip that one course, not flip the whole run onto
        the single-threaded browser fallback. A skipped course simply has no
        description — harmless, and picked up on the next run."""
        if os.environ.get("DASH_FIXTURES"):
            try:
                return parse_detail(fetch(DETAIL.format(id=row["detail"]),
                                          fixture=f"catalog-detail-{row['detail']}.html"))
            except Exception:
                return (None, None, None)
        try:
            from curl_cffi import requests as cffi_requests
            resp = cffi_requests.get(DETAIL.format(id=row["detail"]),
    ...
    for dept_id, code in depts:
        ...
        # Build each course's base entry; queue the ones that still need a
        # live detail fetch (not deduped, not reusable from the prior file).
        dept_entries = []
        to_fetch = []
        for row in rows:
            ...
            if want_detail and existing and not refresh \
                ...
                to_fetch.append((row, entry))
            dept_entries.append(entry)

        # Fetch this department's details concurrently, then fold them in.
        if to_fetch:
            if os.environ.get("DASH_FIXTURES"):
                results = [fetch_detail(row) for row, _ in to_fetch]
            else:
                with ThreadPoolExecutor(max_workers=workers) as pool:
                    results = list(pool.map(lambda re: fetch_detail(re[0]), to_fetch))
            for (row, entry), (desc, prereq, advisory) in zip(to_fetch, results):
                if desc:
        ...
        entries.extend(dept_entries)
        print(f"  {code}: {len(dept_entries)} courses ({len(to_fetch)} detailed)",
              file=sys.stderr)
